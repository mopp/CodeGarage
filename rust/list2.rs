...
impl<T> Node<T> {
    ...
    fn disjoint(&mut self, belong_list: &mut AList<T>)
    {
...
impl<T> AList<T> {
    ...
    fn push_front<'a>(&mut self, new: &'a mut Node<T>)
    {
        match self.head {
            None => {
    ...
    fn pop_front<'a>(&mut self) -> Option<&'a mut Node<T>>
    {
        match self.head {
            None => None,
            Some(shared) => {
    ...
    fn push_back<'a>(&mut self, new: &'a mut Node<T>)
    {
        match self.tail {
            None => {
                self.reset_links(Node::make_link(new));
